...
const fs = require('fs');
const { getProductById } = require('./apiProducts.js');
...
const updateCart = async (cart, identificador) => {
	const arrayCart = await getCart();
...
const deleteProductById = async (cart, identificador) => {
	const arrayCart = await getCart();
	const stamp = new Date().toLocaleString('es-AR');
	const id = parseInt(identificador);
...
const deleteCart = async (identificador) => {
	const arrayCart = await getCart();
...
const getCartProductsById = async (identificador) => {
	const arrayCart = await getCart();
	const id = parseInt(identificador.id);
...
const getCartById = async (identificador) => {
	const arrayCart = await getCart();
	const id = parseInt(identificador.id);
	const hallado = arrayCart.find((e) => e.id === id);
	if (hallado) {
